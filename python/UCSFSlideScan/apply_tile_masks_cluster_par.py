import os
import subprocess
import sys
import fnmatch
import skimage.io as io
import tifffile
from misc.XMLUtils import XMLUtils
import glob
import mahotas as mht
import numpy as np
import multiprocessing as mp

MASK_VAL = 255
N_WORKERS = 4


def get_dirs_dic(root_dir):

    dir_dic = {}

    list_dirs = glob.glob(root_dir + '/*/')
    for ldir in list_dirs:
        if os.path.isdir(ldir):
            if ldir.find('magick_tmp') != -1:
                continue

            # create segmented histo dir
            create_dir_struct(os.path.join(root_dir, ldir))

            # get mask tiles dir path
            mask_dir = os.path.join(ldir, 'mask/final_mask/tiles')
            output_dir = os.path.join(ldir, 'output')
            histo_dir = ''
            seg_dir = os.path.join(ldir, 'heat_map/seg_tiles')

            # iterate inside OUTPUT_DIR to find histo tiles dir
            # we do this the RES(???x???) folder, whose name can change from slice to slice
            for root, dir, files in os.walk(output_dir):
                if fnmatch.fnmatch(root, '*tiles'):  # it's inside /tiles*
                    histo_dir = root

            if not os.path.isdir(mask_dir) or not os.path.isdir(histo_dir):
                print('{} or {} is not a directory'.format(mask_dir, histo_dir))

            dir_dic[histo_dir] = (mask_dir,seg_dir)

    return dir_dic


def apply_masks_worker(dir_dic,dirs):

    proc = mp.current_process()
    pid = proc.pid

    for histo_dir in dirs:
        arr_dir = dir_dic[histo_dir]
        mask_dir = arr_dir[0]
        seg_dir = arr_dir[1]
        if not os.path.isdir(mask_dir) or not os.path.isdir(histo_dir):
            print('{}: {} or {} is not a directory'.format(pid, mask_dir, histo_dir))

        tiles_mask = glob.glob(os.path.join(mask_dir, '*.tif'))
        tiles_histo = glob.glob(os.path.join(histo_dir, '*.tif'))
        nT_mask = len(tiles_mask)
        nT_histo = len(tiles_histo)

        if nT_mask != nT_histo:
            print('{}: Warning: the number o mask and histology tiles is different.'.format(pid))

        for mTile in tiles_mask:
            mask_tile_name = os.path.join(mask_dir, mTile)
            base_name = os.path.basename(mask_tile_name)
            histo_tile_name = os.path.join(histo_dir, base_name)

            # check if files exist
            if not os.path.exists(mask_tile_name):
                print('{}: Error: file {} does not exist'.format(pid, mask_tile_name))
                continue
            if not os.path.exists(histo_tile_name):
                print('{}: Error: file {} does not exist'.format(pid, histo_tile_name))
                continue

            print('{}: Processing {}/{}'.format(pid, histo_tile_name, mask_tile_name))

            mask = io.imread(mask_tile_name)
            img = io.imread(histo_tile_name)

            if mask.ndim > 2:
                mask = mask[:, :, 0]

            if not (mask.shape[0] == img.shape[0] and mask.shape[1] == img.shape[1]):
                tmp = mht.imresize(mask, (img.shape[0], img.shape[1]))
                tmp[tmp < 1] = 0
                tmp[tmp > 0] = 255
                mask = tmp.astype('ubyte')

            size = img.shape[0:2]

            R = img[..., 0].copy()
            G = img[..., 1].copy()
            B = img[..., 2].copy()

            R[mask < MASK_VAL] = 0
            G[mask < MASK_VAL] = 0
            B[mask < MASK_VAL] = 0

            R = R.reshape([size[0], size[1], 1])
            G = G.reshape([size[0], size[1], 1])
            B = B.reshape([size[0], size[1], 1])

            img2 = np.concatenate((R, G, B), axis=2)
            seg_tile_name = os.path.join(seg_dir, base_name)
            mht.imsave(seg_tile_name, img2)

            print('{}: Saved {}'.format(pid, seg_tile_name))


# def apply_masks(root_dir):
#     list_dirs = glob.glob(root_dir+'/*/')
#     for ldir in list_dirs:
#         if os.path.isdir(ldir):
#             if ldir.find('magick_tmp') != -1:
#                 continue
#
#             #create segmented histo dir
#             create_dir_struct(os.path.join(root_dir,ldir))
#
#             #get mask tiles dir path
#             mask_dir = os.path.join(ldir,'mask/final_mask/tiles')
#             output_dir = os.path.join(ldir,'output')
#             histo_dir = ''
#             seg_dir = os.path.join(ldir,'heat_map/seg_tiles')
#
#             #iterate inside OUTPUT_DIR to find histo tiles dir
#             # we do this the RES(???x???) folder, whose name can change from slice to slice
#             for root, dir, files in os.walk(output_dir):
#                 if fnmatch.fnmatch(root, '*tiles'):  # it's inside /tiles*
#                     histo_dir = root
#
#             if not os.path.isdir(mask_dir) or not os.path.isdir(histo_dir):
#                 print('{} or {} is not a directory'.format(mask_dir,histo_dir))
#
#             tiles_mask = glob.glob(os.path.join(mask_dir,'*.tif'))
#             tiles_histo = glob.glob(os.path.join(histo_dir,'*.tif'))
#             nT_mask = len(tiles_mask)
#             nT_histo = len(tiles_histo)
#
#             if nT_mask != nT_histo:
#                 print('Warning: the number o mask and histology tiles is different.')
#
#             for mTile in tiles_mask:
#                 mask_tile_name = os.path.join(mask_dir,mTile)
#                 base_name = os.path.basename(mask_tile_name)
#                 histo_tile_name = os.path.join(histo_dir,base_name)
#
#                 #check if files exist
#                 if not os.path.exists(mask_tile_name):
#                     print('Error: file {} does not exist'.format(mask_tile_name))
#                     continue
#                 if not os.path.exists(histo_tile_name):
#                     print('Error: file {} does not exist'.format(histo_tile_name))
#                     continue
#
#                 print('Processing {}/{}'.format(histo_tile_name,mask_tile_name))
#
#                 mask = io.imread(mask_tile_name)
#                 img = io.imread(histo_tile_name)
#
#                 if mask.ndim > 2:
#                     mask = mask[:,:,0]
#
#                 if not (mask.shape[0] == img.shape[0] and mask.shape[1] == img.shape[1]):
#                     tmp = mht.imresize(mask, (img.shape[0], img.shape[1]))
#                     tmp[tmp < 1] = 0
#                     tmp[tmp > 0] = 255
#                     mask = tmp.astype('ubyte')
#
#                 size = img.shape[0:2]
#
#                 R = img[..., 0].copy()
#                 G = img[..., 1].copy()
#                 B = img[..., 2].copy()
#
#                 R[mask < MASK_VAL] = 0
#                 G[mask < MASK_VAL] = 0
#                 B[mask < MASK_VAL] = 0
#
#                 R = R.reshape([size[0], size[1], 1])
#                 G = G.reshape([size[0], size[1], 1])
#                 B = B.reshape([size[0], size[1], 1])
#
#                 img2 = np.concatenate((R, G, B), axis=2)
#                 seg_tile_name = os.path.join(seg_dir,base_name)
#                 mht.imsave(seg_tile_name,img2)
#
#                 print('Saved {}'.format(seg_tile_name))


def create_dir_struct(root_dir):
    hmap_dir = os.path.join(root_dir,'heat_map')
    if not os.path.exists(hmap_dir):
        os.mkdir(hmap_dir)

    seg_tiles_dir = os.path.join(hmap_dir,'seg_tiles')
    if not os.path.exists(seg_tiles_dir):
        os.mkdir(seg_tiles_dir)


def process_tiles(root_dir):
    #get file information and tiling grid size
    dirs_dic = get_dirs_dic(root_dir)
    nDirs = len(dirs_dic)
    nd_block = int(np.ceil(float(nDirs)/N_WORKERS))
    all_dirs = dirs_dic.keys()

    start_idx = 0
    workers = []
    for i in xrange(N_WORKERS):
        end_idx = start_idx+nd_block
        if (end_idx > nDirs) or (i == (N_WORKERS + 1) and end_idx < nDirs):
            end_idx = nDirs

        file_bundle = all_dirs[start_idx:end_idx]
        start_idx = end_idx
        p = mp.Process(target=apply_masks_worker, args=(dirs_dic,file_bundle))
        workers.append(p)
        p.start()



def main():
    if len(sys.argv) != 2:
        print('Usage: apply_tile_masks_cluster_par.py <root_dir>')
        exit()

    root_dir = str(sys.argv[1])  # abs path to where the images are
    process_tiles(root_dir)

if __name__ == '__main__':
    main()