...
def create_dataset(original_imgs_dir, mask_img_dir, dataset_img_dir, dataset_mask_dir, patch_size, stride):
    ...
    for i in range(len(files)):
        ...
        Rx = sp.get_strided_view(R,F,S)
        Gx = sp.get_strided_view(G,F,S)
        Bx = sp.get_strided_view(B,F,S)

        MCx = sp.get_strided_view(mask_cell,F,S)
        MTx = sp.get_strided_view(mask_thread,F,S)
        MBx = sp.get_strided_view(mask_bkg,F,S)
...
def main():
    ...
    if len(sys.argv) != 7:
        print('Usage: prepare_datasets_taunet <absolute_path_to_imgs> <absolute_path_to_masks> <dataset_path_images> <dataset_path_masks> <patch_size> <stride>')
        exit()
