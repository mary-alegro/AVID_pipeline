...
def mask_worker(file_dic, files):
    ...
    for fi in files:
        ...
        print("{}: Resizing file {}".format(pid,fi))
        status1 = subprocess.call(['convert', '-resize', size_str, fi, str_rname], env=dict(os.environ), stderr=log_err1,
                                 stdout=log_out1)

        if status1 != 0:
            print('{}: Error creating full resolution mask.'.format(pid))
        else:
            #
            # Tile
            #
            ...
            # run tiling process
            print("{}: Tiling file {}".format(pid,str_rname))
            status2 = subprocess.call(['convert', '-crop', str_tile, '+repage', '+adjoin', str_rname, str_tname],
                                     env=dict(os.environ), stderr=log_err2, stdout=log_out2)

            # save metadata (used by export_heatmap_metadata.py)
            if status2 != 0:
                print('{}: There was an error tilling the full resolution mask.'.format(pid))
            else:
...
def process_masks(root_dir):
    ...
    for i in xrange(N_WORKERS):
        ...
        start_idx = end_idx
        p = mp.Process(target=mask_worker, args=(file_dic,file_bundle))
        workers.append(p)
        p.start()
